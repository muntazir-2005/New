// main.m – نسخة آمنة لـ arm64e مع إزالة توقيع PAC قبل DobbyHook
...
#include "dobby.h"
#include <ptrauth.h>         // إن وُجد، وإلا نعرّف الدالة يدوياً

// ========== دالة إزالة توقيع PAC ==========
// إذا لم يتوفر <ptrauth.h>، نستخدم inline assembly
...
static inline void *strip_pac(void *ptr) {
    void *result;
    __asm__ volatile("xpaci %0, %1" : "=r"(result) : "r"(ptr));
    return result;
}
#endif
...
int hook_X509_PUBKEY_set(void **x, void *pkey) { return 1; }
int hook_X509_REQ_check_private_key(void *req, void *pkey) { return 1; }
...
void safeHook(const char *symbol, void *hook_func, void **orig_ptr, const char *desc) {
    ...
    if (addr) {
        addr = strip_pac(addr);   // <-- إزالة التوقيع قبل التمرير
        int ret = DobbyHook(addr, hook_func, orig_ptr);
        if (ret == 0)
            NSLog(@"[Bypass] Hooked %s (%s)", desc, symbol);
        else
            NSLog(@"[Bypass] DobbyHook failed for %s", symbol);
    } else {
...
__attribute__((constructor)) static void init() {
    NSLog(@"[Bypass] Loading protection...");
    applyCHooks();
    applyObjCHooks();
